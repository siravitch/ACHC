...
void setup() {
  ...
  Tic1m.attach(60, tick1m);     //Interrupt every 1m
  Tic10m.attach(600, tick10m);  //Interrupt every 10m
...
void loop() {
  ...
  if (Ticker1m == true) {  //setting timer 1 m
    loopdht();
    getpms();
  ...
  if (Ticker10m == true) {  //setting timer 10 m
